...
def cargar_modelos():
    ...
    if errores:
        st.error("No se pudieron cargar los siguientes modelos:")
        for error in errores:
            st.write(error)
        st.error("Por favor, asegúrate de que los archivos .pkl estén en el directorio de la aplicación")
...
if dataset_original is not None or len(modelos_disponibles) > 0:
...
for service in services_to_fix:
    ...
    with tab5:
        ...
        if dataset_original is None:
            ...
            with col4:
                if len(modelos_disponibles) > 0:
                    st.metric("⭐ Modelos Disponibles", len(modelos_disponibles), help="Número de modelos ML cargados")
                else:
            ...
            **🎯 Situación Actual:**
            ...
            - {"Modelos ML disponibles para predicciones" if len(modelos_disponibles) > 0 else "Sin modelos ML disponibles"}
            ...
            **🚀 Oportunidades:**
            ...
            - {"Predicción temprana permite intervención proactiva" if len(modelos_disponibles) > 0 else "Cargar modelos ML para habilitar predicciones"}
            
            **✅ Próximos Pasos:**
            1. {"Implementar alertas automáticas para clientes de alto riesgo" if len(modelos_disponibles) > 0 else "Cargar modelos ML para habilitar predicciones automáticas"}
